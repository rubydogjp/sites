...
import DevelopTopicPage from "./develop/DevelopTopicPage";
...
const developTopics: TopicCard[] = [
  { slug: "issue", title: "issue", desc: "プロジェクトへの報告・提案", wip: true },
];

function DevelopIndex() {
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
        {developTopics.map((topic) => (
          <Link
            key={topic.slug}
            to={`/develop/${topic.slug}`}
...
export default function DevelopPage() {
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 md:px-6 py-8 md:py-12">
      {currentSlug === "index" ? <DevelopIndex /> : <DevelopTopicPage slug={currentSlug} />}
    </article>
  );
}
